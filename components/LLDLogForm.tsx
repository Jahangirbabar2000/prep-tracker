...
export default function LLDLogForm() {
  ...
  const [lldCategories, setLldCategories]     = useState<string[]>([]);
  const [lldTopics, setLldTopics]             = useState<string[]>([]);
  const inputRef = useRef<HTMLInputElement>(null);

  useEffect(() => {
    fetch('/api/config/options?domain=lld&field=lld_category')
      .then(r => r.json())
      .then((rows: { value: string }[]) => {
        const vals = rows.map(r => r.value);
        setLldCategories(vals);
        if (vals.length > 0) setCategory(vals[0]);
      })
      .catch(() => {});
    fetch('/api/config/options?domain=lld&field=lld_topic')
      .then(r => r.json())
      .then((rows: { value: string }[]) => {
        const vals = rows.map(r => r.value);
        setLldTopics(vals);
      ...
      .catch(() => {});
    fetch('/api/config/options?domain=lld&field=default_link')
      .then(r => r.json())
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      const res = await fetch('/api/problems?domain=lld');
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!pid) {
      const res = await fetch('/api/problems', {
        ...
        body: JSON.stringify({ name, domain: 'lld' }),
      });
    ...
    if (category)      meta.lld_category = category;
    if (topic)         meta.lld_topic    = topic;
    ...
    <form onSubmit={submit} onKeyDown={e => { if (e.key === 'Enter' && (e.metaKey || e.ctrlKey)) { e.preventDefault(); (e.currentTarget as HTMLFormElement).requestSubmit(); } }} className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-1.5 relative">
        ...
        <input
          ...
          placeholder="e.g. Design a parking lot system"
          ...
          <ul className="absolute top-full left-0 right-0 mt-1 bg-surface border border-border rounded-lg shadow-lg z-10 max-h-48 overflow-y-auto">
            {suggestions.map(p => (
              <li key={p.id}>
                ...
                >
                  {p.name}
                  {p.lld_category && <span className="ml-2 text-xs text-muted">{p.lld_category}</span>}
                </button>
              </li>
            ))}
          </ul>
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <textarea
          ...
          placeholder="Key classes, interfaces, tradeoffs… (markdown supported)"
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex gap-2">
          <input
            ...
            placeholder="e.g. Use the Strategy pattern for pricing rules"
            className={inputCls}
          />
          <button
            type="button"
        ...
        <div className="flex gap-3 flex-wrap">
          <div className="flex flex-col gap-1 flex-1 min-w-[140px]">
            ...
            <select value={category} onChange={e => setCategory(e.target.value)} className={inputCls}>
              ...
              {lldCategories.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
          </div>
          <div className="flex flex-col gap-1 flex-1 min-w-[140px]">
            ...
            <select value={topic} onChange={e => setTopic(e.target.value)} className={inputCls}>
              <option value="">— none —</option>
              {lldTopics.map(t => <option key={t} value={t}>{t}</option>)}
            </select>
          </div>
        </div>
