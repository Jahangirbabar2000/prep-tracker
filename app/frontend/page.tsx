...
export default async function FrontendPage({ searchParams }: { searchParams: Promise<Record<string, string>> }) {
  ...
  const [problems, todayRow] = await Promise.all([
    queryAll<Problem>(`
      ...
      FROM problems WHERE domain = 'frontend' ORDER BY created_at DESC
    `),
    queryOne<{ n: number }>(`
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <Link href="/frontend/log" className="hidden md:inline-flex items-center gap-1.5 px-4 py-2 bg-accent text-accent-fg text-sm font-semibold rounded-lg hover:bg-accent-hover transition-colors cursor-pointer">
