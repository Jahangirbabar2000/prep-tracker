// Incremental seed for the existing "system_design" domain: inserts the
// flashcards in scripts/system-design-cards.md, parsed from the
// "## Topic" / "Bucket: <bucket>" / "Link: <url>" / "**Q:** … **A:** …"
// markdown structure.
//
//   node scripts/seed-system-design.mjs         # parse and write
//   node scripts/seed-system-design.mjs --dry   # parse and report, touch nothing
//
// Same contract as scripts/seed-behavioral.mjs — read that header first; only
// the differences are documented here.
//
// One section per source article, and the article maps to a TOPIC, so a section
// is "## <sd_topic>" plus a "Bucket:" line giving its sd_category. Behavioral
// needs only one field per card; this deck needs two, and the bucket is
// many-to-one over topics (Core Concepts holds nine of them), which is why it
// is a line inside the section rather than a second heading level.
//
// "---" is a horizontal rule inside an answer here, not a card separator —
...
const DOMAIN_ID = 'system_design';
const CARDS_FILE = 'scripts/system-design-cards.md';
const BUCKET_FIELD = { key: 'sd_category', label: 'Bucket' };
const TOPIC_FIELD = { key: 'sd_topic', label: 'Topic' };
...
function flushCard() {
  if (mode === 'a' && currentTopic && qBuf) {
    const article = linkByTopic.get(currentTopic) ?? null;
    cards.push({
      topic: currentTopic,
      bucket: bucketByTopic.get(currentTopic) ?? null,
      article,
...
for (const line of lines) {
  ...
  if (topicMatch) {
    ...
    if (/^card style/i.test(currentTopic)) currentTopic = null;
    else if (!seenTopics.includes(currentTopic)) seenTopics.push(currentTopic);
    continue;
  }
  // "Bucket:" declares the sd_category every card in this section belongs to.
  const bucketMatch = line.match(/^Bucket:\s*(.+?)\s*$/);
  if (bucketMatch && mode !== 'a' && currentTopic) {
    bucketByTopic.set(currentTopic, bucketMatch[1]);
    if (!seenBuckets.includes(bucketMatch[1])) seenBuckets.push(bucketMatch[1]);
    continue;
  }
  // "Link:" declares the source article for every card in the current section.
  const linkMatch = line.match(/^Link:\s*(\S+)\s*$/);
  if (linkMatch && mode !== 'a' && currentTopic) {
    linkByTopic.set(currentTopic, linkMatch[1]);
    continue;
  }
...
console.log(`Parsed ${cards.length} cards across ${seenTopics.length} topic(s): ${seenTopics.join(', ')}`);
if (cards.length === 0) throw new Error(`Parsed 0 cards — check ${CARDS_FILE} formatting.`);

const unlinked = seenTopics.filter(t => !linkByTopic.has(t));
if (unlinked.length) throw new Error(`Topic with no "Link:" line: ${unlinked.join(', ')}`);
const unbucketed = seenTopics.filter(t => !bucketByTopic.has(t));
if (unbucketed.length) throw new Error(`Topic with no "Bucket:" line: ${unbucketed.join(', ')}`);
...
// sd_category / sd_topic columns, so write both and stay consistent.
const existing = new Map((await db.execute({
  sql: 'SELECT id, name, notes_text, metadata_json, sd_category, sd_topic FROM problems WHERE domain = ?',
  args: [domain.id],
})).rows.map(r => [r.name, r]));
...
for (const { bucket, topic, q, a } of cards) {
  ...
  if (current) {
    ...
    await db.execute({
      sql: `UPDATE problems SET notes_text = ?, metadata_json = ?, sd_category = ?, sd_topic = ?
            WHERE id = ?`,
  ...
  const row = (await db.execute({
    sql: `INSERT INTO problems
            (name, domain, notes_text, metadata_json, sd_category, sd_topic,
...
const orphans = [...existing.values()].filter(row =>
  seenTopics.includes(row.sd_topic) && !parsedNames.has(row.name),
);
if (orphans.length) {
