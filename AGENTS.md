...
- **Archiving a domain takes it out of rotation, not out of the data.** `archivedDomainIds()` (`lib/domains.ts`) is the one rule; `reviewQueue()` and `forecast()` (`lib/store/queries.ts`) and `buildPracticeSet()` (`lib/practice.ts`) all skip those cards, so an archived domain leaves the queue, the badges, the forecast and every practice set — including its own launcher — while its cards, due dates and attempt history are untouched and come straight back on restore. Anything historical (streak, heatmap, recall rate, "added today") deliberately still counts them. Both admission points must apply the rule, or the `scope: 'due'` equivalence above breaks.
